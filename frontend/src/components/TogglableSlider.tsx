...
const TogglableSlider: React.FC<TogglableSliderProps> = ({ label = "Value", format = "", min = 0, max = 100, step = 1, value = 50, onChange, enabled = true }) => {
  return (
    <div className="relative overflow-hidden p-2">
      <div className={`transition-default ${enabled ? "translate-y-0 opacity-100 delay-200 " : "translate-y-[150%] opacity-0"}`}>
        <Slider label={label} min={min} max={max} step={step} value={value} onChange={onChange} format={format} />
      </div>
